...
def main():
    ...
    vpc.add_name_tag(vpc_id, vars_dev.vpc_name)

    print('Added name ' + vars_dev.vpc_name + ' to ' + vpc_id)
# ok
    ...
    public_subnet_response = vpc.create_subnet(vpc_id, vars_dev.public_cidr_block_ip) 
    ...
    vpc.add_name_tag(public_subnet_id, vars_dev.public_subnet_name) 
    ...
    private_subnet_response = vpc.create_subnet(vpc_id, vars_dev.private_cidr_block_ip) 
    private_subnet_id = private_subnet_response['Subnet']['SubnetId']

    print('Created private subnet ' + private_subnet_id + ' for VPC ' + vpc_id)

    # Add name tag to private subnet
    vpc.add_name_tag(private_subnet_id, vars_dev.private_subnet_name) 
...
##################################################################################
    ...
    # Create a key pair
    try:
        key_pair_response = ec2.create_key_pair(vars_dev.key_pair_name)
        print('Created Key Pair with name ' + vars_dev.key_pair_name + ':' + str(key_pair_response))
    ...
    public_security_group_response = ec2.create_security_group(
        vars_dev.public_security_group_name, public_security_group_description, vpc_id
    )
    ...
    print('Added public access rule to Security Group ' + vars_dev.public_security_group_name)
    ...
    ec2.launch_ec2_instance(vars_dev.ami_id, vars_dev.key_pair_name, 1, 1, public_security_group_id, public_subnet_id, user_data)

    print('Launching Public EC2 Instance using AMI ' + vars_dev.ami_id)
    ...
    private_security_group_response = ec2.create_security_group(vars_dev.private_security_group_name,
    ...
    ec2.launch_ec2_instance(vars_dev.ami_id, vars_dev.key_pair_name, 1, 1, private_security_group_id, private_subnet_id, """""")
